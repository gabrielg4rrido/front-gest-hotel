...
// --- ÍCONES (Substitutos SVG para garantir a funcionalidade) ---
const Calendar = ({ className }: { className: string }) => <svg className={className} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect width="18" height="18" x="3" y="4" rx="2" ry="2"/><line x1="16" x2="16" y1="2" y2="6"/><line x1="8" x2="8" y1="2" y2="6"/><line x1="3" x2="21" y1="10" y2="10"/></svg>;
...
const Download = ({ className }: { className: string }) => <svg className={className} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" x2="12" y1="15" y2="3"/></svg>;
...
export function MyReservationsPage({ onNavigate }: { onNavigate: (page: string) => void }) {
  ...
  const fetchReservations = async () => {
    try {
      ...
      const mappedReservations = response.data.map((apiRes: any) => ({
        ...
        status: new Date(apiRes.dataSaida) < new Date() ? 'completed' : 'confirmed',
        image: `https://placehold.co/600x400/334155/FFFFFF?text=${encodeURIComponent(apiRes.nomeHospede)}`,
      }));

      setReservations(mappedReservations);
    } catch (err) {
      setError('Não foi possível carregar as reservas. Verifique se o servidor da API está a ser executado.');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const handleCancelReservation = async (reservationId: string) => {
    if (window.confirm('Tem a certeza de que deseja cancelar esta reserva?')) {
      try {
        await axios.delete(`http://localhost:3001/api/reservas/${reservationId}`);
  ...
  if (loading) {
    return <div className="text-center p-8">A carregar as suas reservas...</div>;
  }
  if (error) {
      ...
      <div className="space-y-6">
        {reservations.length > 0 ? (
          reservations.map((reservation) => (
            <div key={reservation.id} className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden flex flex-col md:flex-row">
                <img src={reservation.image} alt={reservation.name} className="w-full md:w-1/3 h-48 md:h-auto object-cover" />
                <div className="p-6 flex-1 flex flex-col justify-between">
                    <div>
                        <h3 className="text-xl font-semibold mb-2">{reservation.name}</h3>
                        <p className="text-sm text-gray-600">
                            Check-in: {new Date(reservation.dates.checkIn).toLocaleDateString('pt-BR', { timeZone: 'UTC' })} | 
                            Check-out: {new Date(reservation.dates.checkOut).toLocaleDateString('pt-BR', { timeZone: 'UTC' })}
                        </p>
                        <p className="text-sm text-gray-600">{reservation.guests} hóspede(s)</p>
                    </div>
