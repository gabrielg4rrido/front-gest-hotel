...
import { Calendar, MapPin, Users, Clock, CreditCard, Download, Eye, Star, Plane, Camera } from 'lucide-react';
...
export function MyTravelsPage({ onNavigate }: MyTravelsPageProps) {
  const [travels] = useState<Travel[]>([
    ...
    },
    {
      ...
      rating: 5,
      review: 'Experiência incrível! O quarto era confortável e o atendimento excepcional.',
      photos: [
  ...
  const upcomingTravels = travels.filter(t => t.status === 'confirmed' || t.status === 'pending');
  const pastTravels = travels.filter(t => t.status === 'completed' || t.status === 'cancelled');

  const breadcrumbItems = [
    { label: 'Minhas Viagens', href: '#' }
  ];
        ...
        <div className="flex-1 p-6">
                ...
                <div className="flex items-center gap-1">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      className={`w-4 h-4 ${
                        i < travel.rating! ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'
                      }`}
                    />
                  ))}
                  <span className="text-sm text-gray-600 ml-1">({travel.rating}/5)</span>
                </div>
              )}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              <span>{travel.guests} {travel.guests === 1 ? 'hóspede' : 'hóspedes'}</span>
            </div>
            <div className="flex items-center gap-2 text-sm">
              <Clock className="h-4 w-4 text-gray-400" />
              <span>{calculateDays(travel.dates.checkIn, travel.dates.checkOut)} {calculateDays(travel.dates.checkIn, travel.dates.checkOut) === 1 ? 'diária' : 'diárias'}</span>
            </div>
          </div>

          {travel.additionalServices && travel.additionalServices.length > 0 && (
            <div className="mb-4">
          ...
          {travel.status === 'completed' && travel.photos && travel.photos.length > 0 && (
            <div className="mb-4">
              ...
              <div className="flex gap-2 overflow-x-auto">
                ...
                {travel.photos.length > 4 && (
                  <div className="flex-shrink-0 w-16 h-16 bg-gray-200 rounded-lg flex items-center justify-center">
                    <span className="text-xs text-gray-600">+{travel.photos.length - 4}</span>
                  </div>
                )}
              </div>
            </div>
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">Acompanhe suas viagens e reviva suas melhores experiências</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <Card>
            <CardContent className="p-4 text-center">
              <div className="text-2xl text-green-600">{upcomingTravels.length}</div>
          ...
          <Card>
            <CardContent className="p-4 text-center">
              <div className="text-2xl text-purple-600">
                {travels.reduce((acc, t) => acc + calculateDays(t.dates.checkIn, t.dates.checkOut), 0)}
              </div>
          ...
          <Card>
            <CardContent className="p-4 text-center">
              <div className="text-2xl text-orange-600">
                {pastTravels.length > 0 ? (pastTravels.reduce((acc, t) => acc + (t.rating || 0), 0) / pastTravels.filter(t => t.rating).length).toFixed(1) : '0'}
              </div>
        ...
        <Tabs defaultValue="upcoming" className="space-y-6">
          <TabsList>
            <TabsTrigger value="upcoming">Próximas Viagens ({upcomingTravels.length})</TabsTrigger>
            <TabsTrigger value="past">Viagens Passadas ({pastTravels.length})</TabsTrigger>
          </TabsList>
              ...
              <Card>
                <CardContent className="p-8 text-center">
                  ...
                  <p className="text-gray-600">
                    Suas viagens passadas aparecerão aqui com fotos e avaliações.
                  </p>
                </CardContent>
              </Card>
            )}
