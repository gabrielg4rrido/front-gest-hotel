...
interface RoomListProps {
  ...
  getRoomStatus: (room: Room) => { text: string; variant: 'default' | 'destructive' };
...
export function RoomList({
  ...
  if (loading) {
    return <div className="text-lg text-center py-8">Carregando quartos...</div>;
  }
    ...
    return (
      <div className="w-full text-center py-8">
        <p className="text-gray-500 mb-4">
          {searchData ? 'Nenhum quarto disponível para o período selecionado.' : 'Nenhum quarto encontrado com os filtros aplicados.'}
        </p>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 w-full transition-all duration-300">
      {rooms.map((room) => (
        <RoomCard
          ...
          isReserveDisabled={searchData && !isRoomAvailable(room.id, searchData.checkIn, searchData.checkOut)}
        />
      ))}
    </div>
  );
