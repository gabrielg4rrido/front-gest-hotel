...
export function useRooms({ searchData, filterType, sortBy }: UseRoomsProps) {
  ...
  const filteredAndSortedRooms = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => { // Usar spread `[...]` para não modificar o array original
      switch (sortBy) {
        case 'price':
  ...
  return { rooms: filteredAndSortedRooms, loading, error, refetch: fetchRooms, isRoomAvailable };
}